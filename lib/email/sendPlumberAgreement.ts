...
function buildOfficialAgreementHtml(payload: PlumberAgreementPayload): string {
...
<body style="margin:0;padding:0;background:#0f172a;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif">
  <div style="max-width:600px;margin:0 auto;padding:32px 16px">
    <div style="text-align:center;margin-bottom:24px">
      ...
      <p style="color:#94a3b8;font-size:14px;margin:8px 0 0">Official signed Plumber / plumbing work agreement</p>
    </div>
    <div style="background:#1e293b;border:1px solid #334155;border-radius:12px;overflow:hidden;margin-bottom:20px">
      ...
      <table style="width:100%;border-collapse:collapse">
        ...
        ${row('Plumber name', payload.plumber.companyName || payload.plumber.name)}
        ${row('Site address', payload.siteAddress)}
        ${row('Accepted plumbing rates', payload.acceptedRateLabel)}
    ...
    <p style="color:#fecaca;font-size:13px;line-height:1.5;margin:0 0 16px">
      All funds must flow through the BuilBid payment gateway. Direct cash to the Plumber is prohibited and voids platform guarantees. Accepted rate is fixed. Delay beyond a 10-day grace period: 5% deduction from Plumber payout.
    </p>
...
function buildOfficialAgreementText(payload: PlumberAgreementPayload): string {
  return [
    `Official Signed Agreement — Project #${payload.numericProjectId || payload.projectId} (Plumber / Plumbing Work)`,
    `Project: ${payload.projectTitle}`,
    `Client Name: ${payload.client.name}`,
    `Plumber Name: ${payload.plumber.companyName || payload.plumber.name}`,
    `Site Address: ${payload.siteAddress}`,
    `Accepted plumbing rates: ${payload.acceptedRateLabel}`,
    ...
    'Payment: BuilBid gateway only. Cash to Plumber is prohibited.',
...
/** Emails the signed plumber agreement PDF only to BuilBid official addresses. */
export async function sendOfficialPlumberAgreementEmail(
  payload: PlumberAgreementPayload,
): Promise<void> {
  const { transporter, from } = getMailTransporter();
  const pdfBytes = generatePlumberAgreementPdfBytes(payload);
  ...
  const info = await transporter.sendMail({
    from,
    to,
    subject: plumberAgreementEmailSubject(payload.projectId, payload.numericProjectId),
    text: buildOfficialAgreementText(payload),
    html: buildOfficialAgreementHtml(payload),
    attachments: [
      {
        filename: plumberAgreementFileName(payload.projectId, payload.numericProjectId),
  ...
  console.log('Official plumber agreement emailed:', info.messageId, '→', to.join(', '));
}
