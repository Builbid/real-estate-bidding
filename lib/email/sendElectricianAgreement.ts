...
import {
  ...
  electricianAgreementEmailSubject,
...
function buildOfficialAgreementHtml(payload: ElectricianAgreementPayload): string {
...
<body style="margin:0;padding:0;background:#0f172a;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif">
  <div style="max-width:600px;margin:0 auto;padding:32px 16px">
    <div style="text-align:center;margin-bottom:24px">
      ...
      <p style="color:#94a3b8;font-size:14px;margin:8px 0 0">Official signed Electrician / electrical work agreement</p>
    </div>
    <div style="background:#1e293b;border:1px solid #334155;border-radius:12px;overflow:hidden;margin-bottom:20px">
      ...
      <table style="width:100%;border-collapse:collapse">
        ...
        ${row('Electrician name', payload.electrician.companyName || payload.electrician.name)}
        ${row('Site address', payload.siteAddress)}
        ${row('Accepted electrical rates', payload.acceptedRateLabel)}
    ...
    <p style="color:#fecaca;font-size:13px;line-height:1.5;margin:0 0 16px">
      All funds must flow through the BuilBid payment gateway. Direct cash to the Electrician is prohibited and voids platform guarantees. Accepted rate is fixed. Delay beyond a 10-day grace period: 5% deduction from Electrician payout.
    </p>
...
function buildOfficialAgreementText(payload: ElectricianAgreementPayload): string {
  return [
    `Official Signed Agreement — Project #${payload.numericProjectId || payload.projectId} (Electrician / Electrical Work)`,
    `Project: ${payload.projectTitle}`,
    `Client Name: ${payload.client.name}`,
    `Electrician Name: ${payload.electrician.companyName || payload.electrician.name}`,
    `Site Address: ${payload.siteAddress}`,
    `Accepted electrical rates: ${payload.acceptedRateLabel}`,
    ...
    'Payment: BuilBid gateway only. Cash to Electrician is prohibited.',
...
/** Emails the signed electrician agreement PDF only to BuilBid official addresses. */
export async function sendOfficialElectricianAgreementEmail(
  payload: ElectricianAgreementPayload,
): Promise<void> {
  const { transporter, from } = getMailTransporter();
  const pdfBytes = generateElectricianAgreementPdfBytes(payload);
  ...
  const info = await transporter.sendMail({
    from,
    to,
    subject: electricianAgreementEmailSubject(payload.projectId, payload.numericProjectId),
    text: buildOfficialAgreementText(payload),
    html: buildOfficialAgreementHtml(payload),
    attachments: [
      {
        filename: electricianAgreementFileName(payload.projectId, payload.numericProjectId),
  ...
  console.log('Official electrician agreement emailed:', info.messageId, '→', to.join(', '));
}
