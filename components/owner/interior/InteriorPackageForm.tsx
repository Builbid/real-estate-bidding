...
export function InteriorPackageForm({
  ...
  onChangePackages: (value: InteriorDesignerPackageKind[]) => void;
  onChangeSubOptions: (value: InteriorDesignerSubOptionId[]) => void;
}) {
  function togglePackage(id: InteriorDesignerPackageKind) {
    const enabled = selectedPackages.includes(id);
    const catalog = INTERIOR_DESIGNER_SCOPE_PACKAGES.find((pkg) => pkg.id === id);
  ...
  function toggleSubOption(packageId: InteriorDesignerPackageKind, optionId: InteriorDesignerSubOptionId) {
        ...
        return (
          <div
            ...
            className={cn(
              ...
              open
                ? 'border-blue-500/50 bg-secondary/10'
                ...
                <span className="mt-1 block text-xs font-medium text-slate-700 dark:text-slate-300">
                  {open
                    ? pickedCount > 0
                      ...
                      : 'Select the items interior designers should quote'
