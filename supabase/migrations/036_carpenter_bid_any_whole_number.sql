-- Carpenter bids may be any positive whole number (e.g. 182, 297).
-- Painter and electrician remain exempt. Other services still require 0 or 5.
...
begin
  ...
  skip_multiple_of_five :=
    coalesce(new.service_type::text, '') in ('painter', 'electrician', 'carpenter')
    or coalesce(project_service, '') in ('painter', 'electrician', 'carpenter');
...
comment on function public.validate_bid_rates() is
  'Validates bids.rates JSONB floor keys are positive whole numbers. Multiples of 5 required except painter, electrician, and carpenter bids.';
