-- Electrician bids may be any positive whole number (e.g. 182, 187, 250).
-- Painter remains exempt. Other services still require rates ending in 0 or 5.
...
begin
  ...
  skip_multiple_of_five :=
    coalesce(new.service_type::text, '') in ('painter', 'electrician')
    or coalesce(project_service, '') in ('painter', 'electrician');
...
comment on function public.validate_bid_rates() is
  'Validates bids.rates JSONB floor keys are positive whole numbers. Multiples of 5 required except painter and electrician bids.';
