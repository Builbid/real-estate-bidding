-- Drawing & Design bids may be any positive whole number (e.g. 23, 27, 103).
-- Painter, electrician, and carpenter remain exempt. Other services still require 0 or 5.
...
begin
  ...
  skip_multiple_of_five :=
    coalesce(new.service_type::text, '') in ('painter', 'electrician', 'carpenter', 'drawing_design')
    or coalesce(project_service, '') in ('painter', 'electrician', 'carpenter', 'drawing_design');
...
comment on function public.validate_bid_rates() is
  'Validates bids.rates JSONB floor keys are positive whole numbers. Multiples of 5 required except painter, electrician, carpenter, and drawing_design bids.';
